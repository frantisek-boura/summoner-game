...
const DEFAULT_ZOOM: Vector2 = Vector2(0.5, 0.5)
...
func _physics_process(delta: float) -> void:
	...
	set_zoom(clamp(zoom.lerp(get_zoom() * _current_zoom_target, zoom_speed * delta * (1.0 if not _is_selecting else 0.1)), DEFAULT_ZOOM, SELECT_ZOOM))
