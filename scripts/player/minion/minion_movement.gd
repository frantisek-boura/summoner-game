...
@export_range(10, 1000, 1) var lock_in_acceleration_speed: float = 10
...
func get_idle_position() -> Vector2:
	...
	var idle_position: Vector2 = center_position + Vector2(sin(minion.minion_manager.angle + offset), cos(minion.minion_manager.angle + offset)) * idle_radius
...
func follow_entity(delta: float) -> void:
	var entity_position: Vector2 = minion.minion_manager.entity.global_position
	_direction = minion.global_position.direction_to(entity_position)
